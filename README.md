...
| ファイル | 役割 |
|---|---|
| `Dockerfile` | 2段階ビルド。第1段階はrust:1.88でminaseをビルドし、第2段階はpython:3.11-slimへLishogi-Botを固定コミットで取得して依存パッケージを入れ、minaseのバイナリと起動ラッパーを置く。 |
| `build-image.sh` | minaseのコミットを指定してイメージを作る。ビルド文脈は`git archive`の内容だけなので、未コミットの変更は入らない。イメージには完全ハッシュのタグ、ラベル`org.opencontainers.image.revision`、環境変数`MINASE_COMMIT`が付く。 |
| `minase-lishogi` | `--protocol usi --rules lishogi`を固定してminaseを起動するシェルスクリプト。 |
| `config.yml` | Lishogi-Botの設定。コンテナへ読み取り専用でマウントする。 |
| `compose.yml` | 起動定義。ログは名前付きボリュームに残る。 |
...
- Docker（Compose v2を含む）。
- minaseのリポジトリ。既定では本ディレクトリの隣（`../minase`）を使い、環境変数`MINASE_REPO`で変えられる。
- `bot:play`スコープの認証トークン。対局履歴のない新規アカウントで発行する。昇格は取り消せず、一度でも対局したアカウントは昇格できない。

## 手順

1. 公開に使うminaseのコミットを決め、イメージを作る。

   ```console
   ./build-image.sh <minaseのコミット>
   ```
...
3. 環境変数`LISHOGI_BOT_TOKEN`にトークンを置いて起動し、挑戦の待受に入ったことと認証エラーがないことをログで確かめる。同じアカウントのBotを2つ起動しない。

   ```console
   export LISHOGI_BOT_TOKEN=...
   docker compose up -d
...
運用中のコミットを差し替えるときは、`docker compose down`で止めてから1と3をやり直す。
差し替えはminaseの棋力向上の段階が完了するたびに行い、段階の途中のコミットは使わない。
設定ファイルの変更（`Threads`、`USI_Hash`、受け付ける時間制御、`modes`への`rated`の追加）は、イメージの再ビルドを要せず、`docker compose restart`で反映する。
`compose.yml`のCPU数とメモリ上限は`config.yml`の`Threads`と`USI_Hash`に合わせて変え、両者を一致させる。
